...
public class LayoutEvaluation
{
	...
	/**
	 * Computes the conflict index distance between the embeddings of two
	 * probes. The conflict index distance can be interpreted as the sum of
	 * conflict indices resulting from placing the probes at hypothetical
	 * neighboring spots, ignoring the distance between these spots and the
	 * conflicts generated by other neighbors. The current definition of
	 * conflict index is used (see {@link ConflictIndex}).
	 * 
	 * @param chip chip containing the probes
	 * @param id_1 ID of first probe
	 * @param id_2 ID of second probe
	 * @return the conflict index distance between their embeddings
	 */
	public static double conflictDistance (Chip chip, int id_1, int id_2)
	{
		if (chip instanceof SimpleChip)
			return conflictDistance ((SimpleChip) chip, id_1, id_2);

		throw new IllegalArgumentException ("Unsupported chip type.");
	}
	
	public static double conflictDistance (SimpleChip chip, int id_1,
			int id_2)
	{
		int probe_len, embed_len, base_1, base_2, pos, w, bitmask = 0;
		double dist = 0;
		...
		for (w = -1, pos = 0; pos < embed_len; pos++)
		{
			...
			{
				if ((bitmask & chip.embed[id_2][w]) != 0)
				{
					base_2++;
					dist += ConflictIndex.positionWeight(base_1, probe_len);
				}
			}
			else
			{
				...
				if ((bitmask & chip.embed[id_2][w]) == 0)
					dist += ConflictIndex.positionWeight(base_2, probe_len);
				else
					base_2++;
			}
		}
	...
	// TODO remove weightedDistance methods and use conflictDistance instead
	// since weightedDistance only accounts for the conflicts generated by
	// probe id_2 on id_1 (and not the conflicts generated by id_1 on id_2)  
