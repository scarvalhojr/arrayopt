...
/**
 * This class implements the 2-dimensional Partitioning. The algorithm
 * partitions the chip recursively based on the stated of the embeddings at a
 * particular synthesis step.
 * 
 * <P>The 2-D partitioning uses the first synthesis steps to divide the probe
 * set into sub-sets according to the state of their embeddings (masked or
 * unmasked). The chip is divided into sub-regions proportionally to the number
 * of masked and unmasked embeddings.</P>
 * 
 ...
 * region dimensions (number of rows or columns) is less than {@link #stop_dim}
 * (the stopping dimension can be specified at instantiation time; default value
 * is given by the {@link #DEFAULT_STOP_DIMENSION} constant).</P>
 * 
 ...
 * 
 * <P>The 2-D Partitioning produces a layout in which the first masks are highly
...
public class TwoDimensionalPartitioning implements LayoutAlgorithm
{
	...
	/**
	 * Creates an instance of the 2-D Partitioning algorithm with the specified
	 * filling algorithm.
	 * 
	 ...
	 */
	public TwoDimensionalPartitioning (FillingAlgorithm filler, int stop_dim)
	{
		this.filler = filler;
	...
	public void changeLayout (Chip c)
	{
		...
		horizontalDivide (region, 0, 0, 0, 0, probe_id.length - 1);
	}

	private void horizontalDivide (RectangularRegion r, int step,
			int hpar, int vpar, int start, int end)
	{
		...
		if (r.last_row - r.first_row + 1 <= stop_dim * rows_per_probe)
		{
			...
			verticalDivide (r, step + 1, hpar, vpar, start, end);
			return;
		}
		...
		{
			verticalDivide (r, step, hpar, vpar, start, end);
			return;
		}
		...
		if (hpar == 0)
		{
			...
			verticalDivide (t_region, step, 0, vpar, start, probe_div - 1);

			// assign unmasked probes to bottom sub-region
			verticalDivide (b_region, step, 1, vpar, probe_div, end);
		}
		else
		{
			// assign masked probes to bottom sub-region
			verticalDivide (b_region, step, 1, vpar, start, probe_div - 1);

			// assign unmasked probes to top sub-region
			verticalDivide (t_region, step, 0, vpar, probe_div, end);
		}
	}

	private void verticalDivide (RectangularRegion r, int step,
			int hpar, int vpar, int start, int end)
	{
		...
		if (r.last_col - r.first_col + 1 <= stop_dim)
		{
			...
			horizontalDivide (r, step + 1, hpar, vpar, start, end);
			return;
		}
		...
		if (div_rate < MIN_DIV_RATE)
		{
			horizontalDivide (r, step, hpar, 1 - vpar, start, end);
			return;
		}
		else if (1 - div_rate < MIN_DIV_RATE)
		{
			horizontalDivide (r, step, hpar, vpar, start, end);
			return;
		}
		...
		if (vpar == 0)
		{
			...
			horizontalDivide (l_region, step, hpar, 0, start, probe_div - 1);

			// assign unmasked probes to right sub-region
			horizontalDivide (r_region, step, hpar, 1, probe_div, end);
		}
		else
		{
			// assign masked probes to right sub-region
			horizontalDivide (r_region, step, hpar, 1, start, probe_div - 1);

			// assign unmasked probes to left sub-region
			horizontalDivide (l_region, step, hpar, 0, probe_div, end);
		}
	}
