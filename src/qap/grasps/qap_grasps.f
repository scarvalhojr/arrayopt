...
c          srtcst  - sorts cost 
c          build0  - stage 1 of GRASP construction phase
c          build1  - stage 2 of GRASP construction phase
...
c          swapij  - swaps i and j in permutation
c          randp   - random number generator function
c          readp   - reads problem input data
c          outsol  - prints output summary of solution
c
c     ------------------------------------------------------------------
...
c         nmax   - maximum dimension of qap problem
...
c
c          a      - permutation vector (dim = nmax)
c          b      - permutation vector (dim = nmax)
c          cp     - sparse calculation matrix (dim = nsq)
...
c
c          opta   - best permutation vector (dim = nmax)
c          optb   - best permutation vector (dim = nmax)
c
c     ------------------------------------------------------------------
      integer   opta(n2),optb(n2)
c     ------------------------------------------------------------------
...
c          -------------------------------------------------------------
	   call build0(n,n2,i,j,k,l,seed,alpha,beta,objv,indexd,indexf,
     &                 fdind,cost,a,b)
c          -------------------------------------------------------------
c          Stage 2 of construction phase.
c          -------------------------------------------------------------
	   call build1(n,n2,f,d,alpha,srtc,srtic,a,b,i,j,k,l,seed,
     &                 objv,cp)
c          -------------------------------------------------------------
...
c     ------------------------------------------------------------------
      ...
      subroutine build0(n,n2,i,j,k,l,seed,alpha,beta,objv,indexd,indexf,
     &                  fdind,cost,a,b)
c     ------------------------------------------------------------------
c     build0:  Builds the initial 2 assignments for the GRASP
...
50    return
      ...
      subroutine build1(n,n2,f,d,alpha,srtc,srtic,a,b,i,j,k,l,seed,
     &                  objv,cp)
c     ------------------------------------------------------------------
c     build:  builds a randomized greedy permutation starting from
c             the assignments made in build0.  Permutation is returned
c             in array a(*).
c     ------------------------------------------------------------------
c     Passed input scalars:
c
...
c     ----------------------------------------------------------------
     ...
     &        akm1tn,blm1tn,anm1tn,bnm1tn,scp,oldscp,assz,tl,tk
      real    randp,xrand
c     ----------------------------------------------------------------
c     Initial phase of build1 where the assignments resulting
...
80    continue
      ...
      bnm1tn=(b(n)-1)*n
      do 130 i=1,n-1
	 objv=objv+2*f(anm1tn+a(i))*d(bnm1tn+b(i))
130   continue
      return
      end
...
c     ------------------------------------------------------------------
      ...
      subroutine readp(f,d,n,n2,sparsf,sparsd)
c     ------------------------------------------------------------------
...
c     -------------------------------------------------------------------
      do 10 i = 1, n
           read *, (d((i-1)*n+j), j=1,n)
10    continue
      do 20 i = 1, n
           read *, (f((i-1)*n+j), j=1,n)
20    continue
c     -------------------------------------------------------------------
